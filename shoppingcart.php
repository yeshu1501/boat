...
$connection = mysqli_connect('localhost','root','','boat');
...
switch($_GET["action"]) {
	...
	case "add":
		if(!empty($_POST["quantity"])) {
			$pid=$_GET["pid"];
			$result=mysqli_query($connection,"SELECT * FROM cart WHERE id='$pid'");
...
<HTML>
<HEAD>
<TITLE>Add to cart  for Boat City</TITLE>
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		...
		?>
				...
				<td style="text-align:center;"><a href="shoppingcart.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="images/icon-delete.png" alt="Remove Item" width="20" height="20"/></a></td>
				</tr>
				<?php
...
<div id="product-grid">
	<div class="txt-heading">Boats</div>
	<?php
	$product= mysqli_query($connection,"SELECT * FROM cart ORDER BY id ASC");
	if (!empty($product)) { 
	...
	?>
		<div class="product-item">
			...
			<div class="product-action"><input type="text" class="product-quantity" name="quantity" value="1" size="2" /><input type="submit" value="Add to Cart" class="btnAddAction" />
